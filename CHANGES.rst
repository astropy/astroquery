...
- The ``position`` keyword of ``query_region_maps``, ``query_region_catalogs``, and
  ``query_region_spectra`` is deprecated in favor of ``coordinate``; these methods accept a
  single sky position. [#3609]

heasarc
^^^^^^^

- The ``position`` keyword of ``query_region`` is deprecated in favor of ``coordinate``; the
  method accepts a single sky position. [#3609]

ogle
^^^^

- The ``coord`` keyword of ``query_region`` and ``query_region_async`` is deprecated in favor of
  ``coordinates``; the method accepts one or more positions (as a list of coordinates). [#3609]
