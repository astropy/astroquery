...
Calibration Positional Queries
------------------------------

Positional queries as above can also be made for associated calibrations.  Note that this is a
positional query for data, which then returns calibrations that may be of use in reducing that 
data.  The calibrations themselves may or may not be at that position.

As with other calibration queries, the search is used to find an initial set of data files and then
any relevant calibration files are returned.  The calibration files themselves may or may not match the search
term used.  This is an API equivalent of checking the `Calibrations` tab after performing a search in the
Gemini Observatory Archive.  This call would pair with a call to `~astroquery.gemini.ObservationsClass.query_region` 
using the same parameters for both calls.
...
.. doctest-remote-data::
               ...
               >>> from astropy import coordinates, units
               >>> coord = coordinates.SkyCoord(210.80242917, 54.34875, unit="deg")
               >>> data = Observations.query_calibrations_for_region(coordinates=coord, radius=0.3*units.deg)
               >>> print(data[0:5])
               exposure_time detector_roi_setting ...  release         dec
               ------------- -------------------- ... ---------- ---------------
                    119.9986           Full Frame ... 2008-08-21  54.34877772501
...
Calibration Name Queries
------------------------

Query for calibrations associated with the data returned for a name query.

As with other calibration queries, the search is used to find an initial set of data files and then
any relevant calibration files are returned.  The calibration files themselves may or may not match the search
term used.  This is an API equivalent of checking the `Calibrations` tab after performing a search in the
Gemini Observatory Archive.  This call would pair with a call to `~astroquery.gemini.ObservationsClass.query_object`
...
Calibration Criteria Queries
----------------------------

Additional search terms are available as optional arguments to the 
`~astroquery.gemini.ObservationsClass.query_calibrations_for_criteria` call.  These all have default values of None, 
in which case they will not be considered during the search.  The one exception is ``radius``, which will be set to 
0.3 degrees by default if either ``coordinates`` or ``objectname`` are specified.

Some examples of available search fields are the instrument used, such as GMOS-N, the observation_type, such as BIAS,
and the program ID.  For a complete list of available search fields, see
`~astroquery.gemini.ObservationsClass.query_calibrations_for_criteria`

As with other calibration queries, the search terms are used to find an initial set of data files and then
any relevant calibration files are returned.  The calibration files themselves may or may not match the search
terms used.  This is an API equivalent of checking the `Calibrations` tab after performing a search in the
Gemini Observatory Archive.  This call would pair with a call to `~astroquery.gemini.ObservationsClass.query_criteria`
...
.. doctest-remote-data::
                ...
                >>> from astroquery.gemini import Observations
                >>> data = Observations.query_calibrations_for_criteria('centralspectrum',
                ...                                                     instrument='GMOS-N',
                ...                                                     program_id='GN-CAL20191122',
                ...                                                     observation_type='BIAS',
                ...                                                     filter='r',
...
Calibration Raw Queries
-----------------------

As with observations, for ultimate flexibility, a method is provided for driving the "raw" query that is sent to the
webserver.  For this option, no validation is done on the inputs.  That also means this method may allow
for values or even new fields that were not present at the time this module was last updated.
...
This example is equivalent to doing a web search with
`this example search <https://archive.gemini.edu/searchform/RAW/cols=CTOWEQ/notengineering/GMOS-N/PIname=Hirst/NotFail>`_ 
and then selecting the ``Calibrations`` tab.
...
any relevant calibration files are returned.  The calibration files themselves may or may not match the search
terms used.  This is an API equivalent of checking the `Calibrations` tab after performing a search in the
Gemini Observatory Archive.  This call would pair with a call to `~astroquery.gemini.ObservationsClass.query_raw`
