...
This module provides a python interface for querying the `CDS MOC Service`_ (i.e. the Multi-Order Coverage maps service from the `Centre de Données de Strasbourg (CDS)`_).
...
* A `Multi-Order Coverage object`_ resulting from the union/intersection of all the MOCs of the resulting data sets.
...
1. The sky region from the user (cone, polygon, `Multi-Order Coverage object`_) is converted into a MOC
