...
class MPCClass(BaseQuery):
    ...
    def query_object_async(self, get_query_payload=False, **kwargs):
        """
        ...
        **kwargs :
            ...
            as of 3/6/2018:
                ...
                json :
                    1 for a JSON response, 0 for XML
        """

        if kwargs is None:
            raise InvalidArgumentException("You must provide a parameter to search with")
        ...
        response = self._request('GET', self.MPC_URL, request_payload, auth=(self.MPC_USERNAME, self.MPC_PASSWORD))
        print(response.json())
        return response
