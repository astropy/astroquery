...
class TestTap:
    ...
    def test_query_target_error(self):
        ...
        assert 'This target name cannot be determined with this resolver: SIMBAD' in err.value.args[0] or "Failed "
        "to parse" in err.value.args[0]

        with pytest.raises((ValueError, TableParseError)) as err:
            jwst.query_target(target_name="test", target_resolver="NED",
                              radius=units.Quantity(5, units.deg))
        assert 'This target name cannot be determined with this resolver: NED' in err.value.args[0] or "Failed "
        "to parse" in err.value.args[0]

        with pytest.raises((ValueError, TableParseError)) as err:
            jwst.query_target(target_name="test", target_resolver="VIZIER",
                              radius=units.Quantity(5, units.deg))
        assert 'This target name cannot be determined with this resolver: VIZIER' in err.value.args[0] or "Failed "
        "to parse" in err.value.args[0]
