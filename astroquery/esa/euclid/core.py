...
class EuclidClass(TapPlus):
    ...
    REQUESTED_TILE_ID = "Missing required argument: 'tile_id'"
    REQUESTED_OBSERVATION_ID_AND_TILE_ID = "Incompatible: 'observation_id' and 'tile_id'. Use only one."
    REQUESTED_PRODUCT_TYPE = "Missing required argument: 'product_type'"
    REQUESTED_GENERIC = "Missing required argument"
    REQUESTED_RADIUS = "Radius cannot be greater than 30 arcminutes"
    ...
    def get_obs_products(self, *, id=None, schema="sedm", product_type=None, product_subtype="STK", filter="VIS",
        ...
        if product_type is None:
            raise ValueError(self.REQUESTED_PRODUCT_TYPE)
    ...
    def __get_tile_catalogue_list(self, *, tile_index=None, product_type=None, verbose=False):
        ...
        if tile_index is None:
            raise ValueError(self.REQUESTED_TILE_ID)
        if product_type is None:
            raise ValueError(self.REQUESTED_PRODUCT_TYPE)
    ...
    def get_product_list(self, *, observation_id=None, tile_index=None, product_type=None, verbose=False):
        ...
        if observation_id is not None and tile_index is not None:
            raise ValueError(self.REQUESTED_OBSERVATION_ID_AND_TILE_ID)

        if observation_id is None and tile_index is None:
            raise ValueError(self.REQUESTED_OBSERVATION_ID + "; " + self.REQUESTED_TILE_ID)
    ...
    def get_cutout(self, *, file_path=None, instrument=None, id=None, coordinate, radius, output_file=None,
        ...
        if file_path is None or instrument is None or id is None or coordinate is None or radius is None:
            raise ValueError(self.REQUESTED_GENERIC)

        # Parse POS
        radius_deg = Angle(self.__get_quantity_input(radius, "radius")).to_value(u.deg)
        if radius_deg > 0.5:
            raise ValueError(self.REQUESTED_RADIUS)
        coord = self.__get_coord_input(coordinate, "coordinate")
        ra_hours, dec = commons.coord_to_radec(coord)
        ra = ra_hours * 15.0  # Converts to degrees
