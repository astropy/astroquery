...
def test_cone_search_errors():
    ...
    with pytest.raises(ValueError, match="Invalid ra or dec column names: 'None' and 'dec'"):
        job = tap.cone_search(SKYCOORD, radius=RADIUS, table_name="", ra_column_name=None, dec_column_name="dec",
                              output_format='votable')
    with pytest.raises(ValueError, match="Invalid ra or dec column names: '' and 'dec'"):
        job = tap.cone_search(SKYCOORD, radius=RADIUS, table_name="", ra_column_name="", dec_column_name="dec",
                              output_format='votable')

    with pytest.raises(ValueError, match="Invalid ra or dec column names: 'ra' and 'None'"):
        job = tap.cone_search(SKYCOORD, radius=RADIUS, table_name="", ra_column_name="ra", dec_column_name=None,
                              output_format='votable')

    with pytest.raises(ValueError, match="Invalid ra or dec column names: 'ra' and ''"):
        job = tap.cone_search(SKYCOORD, radius=RADIUS, table_name="", ra_column_name="ra", dec_column_name="",
                              output_format='votable')
