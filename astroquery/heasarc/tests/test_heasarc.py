...
OBJ_LIST = ["182d38m08.64s 39d24m21.06s",
...
def test_query_region_cone(coordinates, radius):
    ...
    query = Heasarc.query_region(coordinates, table='suzamaster', spatial='cone', radius=radius,
    ...
    assert "SELECT *,DISTANCE(POINT('ICRS',ra,dec), POINT('ICRS',182.63" in query
    assert "FROM suzamaster WHERE CONTAINS(POINT('ICRS',ra,dec),CIRCLE('ICRS',182.63" in query
...
def test_query_region_box(coordinates, width):
    query = Heasarc.query_region(coordinates, table='suzamaster', spatial='box', width=2 * u.arcmin,
                               columns='*', get_query_payload=True)
...
def test_query_region_polygon(polygon):
    ...
    query1 = Heasarc.query_region(table="suzamaster", spatial="polygon", polygon=polygon,
                                columns='*', get_query_payload=True)
    query2 = Heasarc.query_region("ngc4151", table="suzamaster", spatial="polygon", polygon=polygon,
                                columns='*', get_query_payload=True)
...
def test_query_allsky():
    query1 = Heasarc.query_region(table="suzamaster", spatial="all-sky",
                                columns='*', get_query_payload=True)
    query2 = Heasarc.query_region("m31", table="suzamaster", spatial="all-sky",
                                columns='*', get_query_payload=True)
...
def test_spatial_invalid(spatial):
    with pytest.raises(ValueError):
        Heasarc.query_region(OBJ_LIST[0], table='invalid_spatial', columns='*', spatial=spatial)
...
def test_get_link():
    ...
    with pytest.raises(ValueError, match='Unknown table name:'):
        Heasarc.get_links(Table({'__row':[1,2,3.]}), tablename=1)

    with pytest.raises(ValueError, match='No __row column found'):
        Heasarc.get_links(Table({'id':[1,2,3.]}), tablename='xray')
...
def test_download_data__wronghost():
    with pytest.raises(ValueError, match='host has to be one of heasarc, sciserver, aws'):
...
def test_download_data__missingcolumn(host):
    ...
    with pytest.raises(ValueError, match=f'No {host_col} column found in the table. Call ~get_links first'):
        Heasarc.download_data(Table({'id':[1]}), host=host)

def test_download_data__outside_sciserver():
    with pytest.raises(FileNotFoundError, match='No data archive found. This should be run on Sciserver'):
        Heasarc.download_data(Table({'sciserver':['some-link']}), host='sciserver')
...
def _s3_mock(mocker):
    with mock_aws():
        ...
        s3_client.put_object(Bucket=s3_bucket, Key=s3_key1, Body='my content')
        s3_client.put_object(Bucket=s3_bucket, Key=s3_key2, Body='my other content')
        s3_client.put_object(Bucket=s3_bucket, Key=s3_key3, Body='my other content')
        yield conn
