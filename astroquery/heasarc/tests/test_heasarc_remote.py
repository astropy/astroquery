...
class TestHeasarc:
    ...
    def test_list_tables__keywords(self):
        ...
        tables = list(Heasarc.tables(keywords=['swift', 'rosat'], master=True)['name'])
        assert 'swiftmastr' in tables
    ...
    def test_get_links__wrongtable(self):
        with pytest.raises(ValueError, match="Unknown table name:"):
            Heasarc.get_links(Table({"__row": [1, 2, 3.0]}), tablename="wrongtable")

    def test_get_links__xmmmaster(self):
        links = Heasarc.get_links(Table({"__row": [4154, 4155]}), tablename="xmmmaster")
        assert len(links) == 2
    ...
    def test_deprecated_pars(self, pars):
        ...
        with pytest.warns(AstropyDeprecationWarning):
            if keyword == 'mission':
                ...
                Heasarc.query_region(pos, mission='xmmmaster', **{keyword:value})
