...
def test_query_allsky():
    query1 = Xamin.query_region(table="suzamaster", spatial="all-sky",
                                columns='*', get_query_payload=True)
    query2 = Xamin.query_region("m31", table="suzamaster", spatial="all-sky",
                                columns='*', get_query_payload=True)
...
def test_spatial_invalid(spatial):
    with pytest.raises(ValueError):
        Xamin.query_region(OBJ_LIST[0], table='invalid_spatial', columns='*', spatial=spatial)

def test_no_table():
    with pytest.raises(InvalidQueryError):
        Xamin.query_region("m31", spatial='cone', columns='*')
...
def test_download_data__missingcolumn(host):
    ...
    with pytest.raises(ValueError, match=f'No {host_col} column found in the table. Call ~get_links first'):
        Xamin.download_data(Table({'id':[1]}), host=host)
...
def test_download_data__outside_sciserver():
    with pytest.raises(FileNotFoundError, match='No data archive found. This should be run on Sciserver'):
        Xamin.download_data(Table({'sciserver':['some-link']}), host='sciserver')
...
def test_s3_mock_file(s3_mock):
    ...
    Xamin.enable_cloud(profile=False)
    Xamin.download_data(links, host='aws', location='.')
