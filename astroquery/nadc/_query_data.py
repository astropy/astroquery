...
def _openapi_path(path: str = "") -> str:
    ...
    if not suffix:
        return _QUERY_DATA_OPENAPI_PREFIX
    return f"{_QUERY_DATA_OPENAPI_PREFIX}/{suffix}"
...
class QueryDataBaseQuery(BaseQuery):
    ...
    def _openapi_url(self, path: str = "") -> str:
        return self._service_url(_openapi_path(path))
    ...
    def _request_coordinate_groups(
        ...
        *,
        ...
        url = self._openapi_url(
            f"catalogs/{catname}/tables/{table}/coordinate_groups"
        )
        if get_query_payload:
            params, headers = self._auth_params_headers()
            return {"method": "GET", "url": url, "params": params, "headers": headers}
    ...
    def _request_table_conesearch(
        ...
        *,
        ...
        catname = catalog or self.catalog
        url = self._openapi_url(
            f"catalogs/{catname}/tables/{table}/vo/conesearch"
        )
    ...
    def _file_download_request_config(
        ...
        *,
        ...
        url = self._endpoint_from_metadata(
            endpoint,
            default_path=_openapi_path(f"catalogs/{catname}/file"),
        )
    ...
    def _request_batch_download(
        ...
        *,
        ...
        url = self._endpoint_from_metadata(
            endpoint,
            default_path=_openapi_path(f"catalogs/{catname}/download"),
        )
        json_payload = dict(payload)
        if get_query_payload:
            params, headers = self._auth_params_headers()
