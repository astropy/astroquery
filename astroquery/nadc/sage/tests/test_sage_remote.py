...
from .. import conf
from ..core import SageClass


pytestmark = pytest.mark.remote_data


SUPPORTED_CATALOGS = {"SAGES-DR1", "SAGES-StellarParameters"}
...
def test_list_catalogs_remote():
    ...
    if "catname" in result.colnames and len(result) > 0:
        assert {_as_text(value) for value in result["catname"]} <= SUPPORTED_CATALOGS


def test_query_uv_sources_remote():
    client = _skip_if_remote_unconfigured()
    result = client.query_uv_sources(
        output_format="json",
        max_rows=1,
        page_size=1,
        cache=False,
