...
class TestEso:
    ...
    def test_query_tap_service(self):
        ...
        assert len(t) == eso.ROW_LIMIT, (
            f"Table length is {lt}, expected {eso.ROW_LIMIT}"
        )
    ...
    def test_apex_retrieval(self):
        ...
        tblb = eso.query_apex_quicklooks(
            column_filters={"project_id": "E-095.F-9802A-2015"}
        )
        tbla = eso.query_apex_quicklooks(column_filters={"prog_id": "095.F-9802(A)"})
    ...
    def test_sgrastar(self):
        ...
        with pytest.warns(MaxResultsWarning):
            result_i = eso.query_instrument(
                "midi", cone_ra=266.41681662, cone_dec=-29.00782497, cone_radius=1.0
            )
    ...
    def test_sgrastar_column_filters(self):
        ...
        result1 = eso.query_surveys(
            "sphere, vegas",
            columns=(
                "obs_collection, calib_level, "
                "multi_ob, filter, s_pixel_scale, instrument_name"
            ),
            column_filters={"calib_level": "= 3", "multi_ob": "like '%M%'"},
        )

        result2 = eso.query_surveys(
            "sphere, vegas",
            columns=(
                "obs_collection, calib_level, "
                "multi_ob, filter, s_pixel_scale, instrument_name"
            ),
            column_filters={"calib_level": 3, "multi_ob": "M"},
        )
    ...
    def test_each_instrument_sgrastar(self, instrument):
        ...
        try:
            ...
            assert len(result) > 0, (
                f"query_instrument({instrument}) returned no records"
            )
    ...
    def test_mixed_case_instrument(self, tmp_path):
        ...
        with pytest.warns(MaxResultsWarning):
            result1 = eso.query_instrument(
                "midi", cone_ra=266.41681662, cone_dec=-29.00782497, cone_radius=1.0
            )
            result2 = eso.query_instrument(
                "MiDi", cone_ra=266.41681662, cone_dec=-29.00782497, cone_radius=1.0
            )
        assert all(result1.values_equal(result2))
    ...
    def test_main_sgrastar(self):
        ...
        with pytest.warns(MaxResultsWarning):
            result = eso.query_main(
                column_filters={"target": "SGR A", "object": "SGR A"}
            )
